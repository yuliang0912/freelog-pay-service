...
module.exports = class FeatherTransferEvent extends Subscription {
    ...
    async subscribe() {
        ...
        const lastBlock = await dal.featherTransferProvider.getMaxBlockNumber()

        const fromBlock = lastBlock ? lastBlock.blockInfo.blockNumber + 1 : 0

        ethClient.CoinContract.getPastEvents('Transfer', {fromBlock}).then(events => events.map(item => new Object({
            ...
            if (eventArray.length) {
                dal.featherTransferProvider.batchAddTransferInfos(eventArray)
